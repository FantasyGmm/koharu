...
- Koharu は compute capability 7.0 以上の NVIDIA GPU をサポートし、V100 のような Volta 世代も含みます
- Koharu はメインの CUDA パイプライン向けに CUDA Toolkit 13.0 を同梱しています
...
!!! note
    ...
    Windows では、任意のローカル llama.cpp CUDA ランタイムが依然として upstream の CUDA 13.1 パッケージを使うため、V100 クラスの環境ではローカル LLM 推論は Vulkan または CPU にフォールバックします。
