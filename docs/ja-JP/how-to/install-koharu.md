...
- NVIDIA ドライバが同梱された CUDA 13.0 メインランタイムに対応していると確認できない場合、Koharu は CPU にフォールバックする

CUDA 対応環境では、必要なランタイム部品を手作業でライブラリパス設定しなくてもよいように、Koharu が自前で同梱・初期化します。

!!! note

    NVIDIA ドライバは最新に保ってください。Koharu はメインパイプライン向けの CUDA 13.0 対応を確認し、ドライバが古い場合は CPU にフォールバックします。Windows では任意のローカル llama.cpp CUDA ランタイムが引き続き upstream の CUDA 13.1 パッケージを使います。
