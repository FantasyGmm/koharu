...
Koharu が同梱 CUDA ランタイムへの対応を確認できない場合、これは想定された挙動です。メインパイプラインでは CUDA 13.0 を指します。

現在のランタイム挙動は次の通りです。

- NVIDIA ドライバを検出する
- ドライバ互換性を問い合わせる
- メインパイプライン向けにドライバが CUDA 13.0 対応を報告した場合のみ CUDA を使う
- Windows では、upstream の CUDA 13.1 ローカル llama.cpp パッケージも使える場合に限ってローカル LLM で CUDA を使う
